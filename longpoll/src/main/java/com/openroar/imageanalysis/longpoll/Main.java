...
/**
 * Created by jamesjackson on 2/24/17.
 */
...
public class Main {
    ...
    private static void downloadFromDropboxDelete(DbxClientV2 client, String filePath) throws DbxException,
            IOException {
        Path p = Paths.get(filePath);
        String fileName = p.getFileName().toString();
        FileOutputStream outputStream = new FileOutputStream(Config.getProperty("darknet_path") + "/images_download/" + fileName);
        try {
            FileMetadata metadata = client.files().download(filePath).download(outputStream);
        } finally {
            outputStream.close();
            Jedis jedis = new Jedis("localhost");
            jedis.rpush("new_image_queue", fileName);
        ...
        while (true) {
            ...
            for (Metadata metadata : result.getEntries()) {
                ...
                if (metadata instanceof FileMetadata) {
                    ...
                    downloadFromDropboxDelete(client, metadata.getPathLower());
